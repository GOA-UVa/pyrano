...
_APPARENT_ELEVATION_LIMIT_LIGHT =  -6
...
def get_solar_day_lims(dt: datetime, lat: float, lon: float) -> Tuple[datetime, datetime]:
    ...
    mask = solpos['apparent_elevation'].values > _APPARENT_ELEVATION_LIMIT_LIGHT
...
def filter_solar_day(df, lat, lon) -> pd.DataFrame:
    ...
    df = df[solpos['apparent_elevation'].values > _APPARENT_ELEVATION_LIMIT_LIGHT]
    return df
